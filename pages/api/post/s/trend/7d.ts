...
const fnn = `${PostsOd7DaysHot.name} (API)`;
...
export default async function PostsOd7DaysHot(req: NextApiRequest, res: NextApiResponse) {
    const { method } = req;
    if ('GET' !== method) {
        response405(req, res);
        return;
    ...
    try {
        ...
        const conditions = [{ status: { $gt: 0 }, createdTimeBySecond: { $gt: getTimeBySecond() - 7 * 24 * 60 * 60 } }, ('string' === typeof channelId && !['', 'all'].includes(channelId)) ? { channelId: channelId } : {}];
        const pipeline = [
