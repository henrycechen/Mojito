...
export default async function GetCommentsByParentId(req: NextApiRequest, res: NextApiResponse) {
    ...
    if ('P1234ABCDX' === req.query?.parentId) {
        res.send([
    ...
    if ('C12345ABCDE' === req.query?.parentId) {
        res.send([{
            ...
            content: `周杰倫在臺北縣林口鄉（今新北市林口區）長大[5]，為家中的獨生子。父親周耀中，當時任教於蘆洲國中，教授生物[6]；母親葉惠美則是林口國中美術教師。14歲時父母離異，由父親擔任監護人，年滿18歲後選擇與母親共同生活[7]。周杰倫曾在台灣民視新聞台由胡婉玲主持的節目《台灣演義》專訪中澄清《爸，我回來了》只是對社會上家暴現象的@_@感慨，並非指涉父母間的狀況；父親方面的親戚也曾質疑過他，他還為此向親戚們澄清，為此誤解抱歉過[8]。`,
            cuedMemberInfoArr: [],
            status: 201,
            totalLikedCount: 1,
            totalDislikedCount: 1
        },
        {
            ...
            avatarImageUrl: '',
            createdTimeBySecond: 1673485211,
            content: `周杰倫自小對音樂表現出濃厚的興趣，並且喜歡模仿歌星、演員表演和變魔術。3歲開始學習鋼琴。周杰倫國小時住在臺北市光華商場附近，就讀忠孝國小。國中時就讀金華國中[9]，此時期他的父母因長年爭執而決議離婚，使周杰倫的性情大受影響。除了音樂外，周杰倫熱愛籃球，在國中參加籃球隊，結識學長陳建州。@林北`,
            cuedMemberInfoArr: [{ memberId: 'M1234ABCE', nickname: '林北' }],
            status: 201,
            totalLikedCount: 1,
            totalDislikedCount: 1
        }]);
